...
import unittest
#单元测试要继承的父类为unittest.TestCase
class Test(unittest.TestCase):
    #重写setUp方法用来完成测试用例执行前的初始化工作
    def setUp(self):
        self.number = 10
        self.number = int(self.number)
    #所有测试用例都以‘test_’开头
    ##可使用unittest.TestCase类下面的各种断言方法用于对测试结果的判断
    #assertEqual断言第一个参数为被测试目标，第二个参数为期待值，第三个参数msg为若被测试目标不符合期待值需要输出的语句
    def test_1(self):
        print(self.number)
        self.assertEqual(self.number,10,msg='You input is not 10')
    def test_2(self):
        print(self.number)
        self.assertEqual(self.number,20,msg='You input is not 20')
    def test_3(self):
        print(self.number)
        self.assertEqual(self.number, 30, msg='You input is not 30')

    @unittest.skip('暂时跳过该检测')
    def test_4(self):
        print(self.number)
        self.assertEqual(self.number,40,msg='You input is not 40')
 # 重写tearDown()方法用于测试用例执行之后的善后工作。
    def tearDown(self):
        print('Test over')
